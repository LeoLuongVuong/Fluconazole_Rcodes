...
Fluc_dosing_events <- Fluc_dosing_events %>%
  mutate(
    maintenance_dose = ifelse(ID %in% distinctID_maintenance_dose, 3, maintenance_dose)
  )
# Also need to assign 3 to all missing maintenance_dose_interval
Fluc_dosing_events$maintenance_dose[is.na(Fluc_dosing_events$maintenance_dose)]<-3
...
# Create common title
title <- ggdraw() + 
draw_label("CRRT", fontface = "bold", size = 12) #
...
# Create common title
title <- ggdraw() + 
draw_label("Non-CRRT", fontface = "bold", size = 12) #change title according to Erwin's comments
...
#grid_BW_hybrid_DAY114 #comment out 161123 since it's not relevant anymore
...
PTA_Cmin80 <- ggplot(PTA_BW, aes(x = factor(CRRT), y = PTA_Cmin_80)) +
  ...
  scale_y_continuous(limits = c(-0.01, 2.4), breaks = seq(0, 2.4, by = 0.1),expand = c(0,0)) +

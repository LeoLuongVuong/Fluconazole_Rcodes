...
title <- ggdraw() + 
  ...
  draw_label("Probability of target attainment across the range of CKDEPI in non CRRT patients", fontface = "bold", size = 12)
...
title <- ggdraw() + 
  #draw_label("Probability of fAUC 200 mgxh/L target attainment", fontface = "bold", size = 12)
draw_label("Probability of target attainment across the range of body weight in non CRRT patients", fontface = "bold", size = 12)
...
title <- ggdraw() + 
  #draw_label("Probability of fAUC 200 mgxh/L target attainment", fontface = "bold", size = 12)
draw_label("Probability of target attainment across the range of body weight in CRRT patients", fontface = "bold", size = 12)
...
grid_BW_hybrid_DAY114 <- plot_grid(grid_BW_noCRRT, grid_BW_CRRT,nrow = 2, align = "v")
grid_BW_hybrid_DAY114
...
#### ROC curves pop_sim_std & opt datasets
...
ROC_AUC_std_DAY1 <- pROC::roc(TA_std_day1$TA,
                        TA_std_day1$BW,
                        auc.polygon = FALSE, 
...
# Calculate 95% CI
AUC_std <- pROC::auc(ROC_AUC_std_DAY1, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)

AUC_firstpart_std <- round(as.numeric(sub(".*(: *)", "//1", AUC_std)), 3)

CI_AUC_std <- pROC::ci.auc(ROC_AUC_std_DAY1, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)

AUC_anotherpart_std <- sub(".*(: *)", "//1", CI_AUC_std)

AUC_secondpart_std <- as.numeric(substr(AUC_anotherpart_std[1], 1, 5))

AUC_thirdpart_std <- as.numeric(substr(AUC_anotherpart_std[3], 1, 5))

AUC_label_std_DAY1 <- paste0("AUC:", AUC_firstpart_std, "(", AUC_secondpart_std, "-", AUC_thirdpart_std, ")", sep = "") 

# For recommended dosing
TA_opt_day1 <- pop_opt_dos %>% filter(DAY == 1) %>% mutate(TA = ifelse(fAUC >= 200, 1, 0))

ROC_AUC_opt_DAY1 <- pROC::roc(TA_opt_day1$TA,
                        TA_opt_day1$BW,
                        auc.polygon = FALSE,
                        max.auc.polygon = FALSE,
                        grid = FALSE,
...
AUC_opt <- pROC::auc(ROC_AUC_opt_DAY1, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)

AUC_firstpart_opt <- round(as.numeric(sub(".*(: *)", "//1", AUC_opt)), 3)

CI_AUC_opt <- pROC::ci.auc(ROC_AUC_opt_DAY1, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)

AUC_anotherpart_opt <- sub(".*(: *)", "//1", CI_AUC_opt)

AUC_secondpart_opt <- as.numeric(substr(AUC_anotherpart_opt[1], 1, 5))

AUC_thirdpart_opt <- as.numeric(substr(AUC_anotherpart_opt[3], 1, 5))

AUC_label_opt_DAY1 <- paste0("AUC:", AUC_firstpart_opt, "(", AUC_secondpart_opt, "-", AUC_thirdpart_opt, ")", sep = "")

# Convert roc objects to data frames
roc_std_DAY1 <- pROC::coords(ROC_AUC_std_DAY1)
roc_opt_DAY1 <- pROC::coords(ROC_AUC_opt_DAY1)
...
roc_curve_day1 <- ggplot() +
  ...
  annotate(geom = "text", x = 0.60, parse = TRUE, y = 0.30, size = 4, label = AUC_label_std_DAY1, color = "orange") +
  annotate(geom = "text", x = 0.60, parse = TRUE, y = 0.10, size = 4, label = AUC_label_opt_DAY1, color = "steelblue") +
...
#combined_plot_DAY1 <- grid.arrange(annotation, roc_curve_day1, ncol = 1, heights = c(0.02, 0.98)) 
...
#ggsave("roc_opt_DAY1.png", combined_plot_DAY1, dpi = 300, width = 8.27, height = 10.3, units = "in", limitsize = FALSE)
...
analysis1_DAY2 <- sdtab_pop_std_2706_trough %>% filter(DAY == 2) %>% mutate(TA = ifelse(fAUC >= 200, 1, 0))

ROC_AUC_std_DAY2 <- pROC::roc(analysis1_DAY2$TA,
                        analysis1_DAY2$BW,
...
AUC_std <- pROC::auc(ROC_AUC_std_DAY2, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)
AUC_firstpart_std <- round(as.numeric(sub(".*(: *)", "//1", AUC_std)), 3)
CI_AUC_std <- pROC::ci.auc(ROC_AUC_std_DAY2, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)
AUC_anotherpart_std <- sub(".*(: *)", "//1", CI_AUC_std)
AUC_secondpart_std <- as.numeric(substr(AUC_anotherpart_std[1], 1, 5))
AUC_thirdpart_std <- as.numeric(substr(AUC_anotherpart_std[3], 1, 5))
AUC_label_AUC_BW1_DAY2 <- paste0("AUC:", AUC_firstpart_std, "(", AUC_secondpart_std, "-", AUC_thirdpart_std, ")", sep = "")

# For recommended dosing - DAY 2
analysis_DAY2 <- pop_opt_dos %>% filter(DAY == 2) %>% mutate(TA = ifelse(fAUC >= 200, 1, 0))

ROC_AUC_opt_DAY2 <- pROC::roc(analysis_DAY2$TA,
                        analysis_DAY2$BW,
                        auc.polygon = FALSE, max.auc.polygon = FALSE, grid = FALSE,
                        ci = TRUE, boot.n = 1000, ci.alpha = 0.95, stratified = TRUE,
                        plot = FALSE, print.auc = TRUE, print.auc.x = 0.2, print.auc.y = 0.3,
...
AUC_opt <- pROC::auc(ROC_AUC_opt_DAY2, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)
AUC_firstpart_opt <- round(as.numeric(sub(".*(: *)", "//1", AUC_opt)), 3)
CI_AUC_opt <- pROC::ci.auc(ROC_AUC_opt_DAY2, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)
AUC_anotherpart_opt <- sub(".*(: *)", "//1", CI_AUC_opt)
AUC_secondpart_opt <- as.numeric(substr(AUC_anotherpart_opt[1], 1, 5))
AUC_thirdpart_opt <- as.numeric(substr(AUC_anotherpart_opt[3], 1, 5))
AUC_label_AUC_BW_DAY2 <- paste0("AUC:", AUC_firstpart_opt, "(", AUC_secondpart_opt, "-", AUC_thirdpart_opt, ")", sep = "")

# Convert roc objects to data frames
roc1_DAY2 <- pROC::coords(ROC_AUC_std_DAY2)
roc_DAY2 <- pROC::coords(ROC_AUC_opt_DAY2)

# Plot ROC curves - DAY 2
Plot_ROC_AUC_opt_DAY2 <- ggplot() +
...
# Combine the annotation and plot using grid.arrange
combined_plot_DAY2 <- grid.arrange(annotation, Plot_ROC_AUC_opt_DAY2, ncol = 1, heights = c(0.02, 0.98))
...
analysis1_DAY7 <- sdtab_pop_std_2706_trough %>% filter(DAY == 7) %>% mutate(TA = ifelse(fAUC >= 200, 1, 0))

ROC_AUC_std_DAY7 <- pROC::roc(analysis1_DAY7$TA,
                        analysis1_DAY7$BW,
                        auc.polygon = FALSE, max.auc.polygon = FALSE, grid = FALSE,
                        ci = TRUE, boot.n = 1000, ci.alpha = 0.95, stratified = TRUE,
                        plot = FALSE, print.auc = TRUE, print.auc.x = 0.2, print.auc.y = 0.3,
...
AUC_std <- pROC::auc(ROC_AUC_std_DAY7, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)
AUC_firstpart_std <- round(as.numeric(sub(".*(: *)", "//1", AUC_std)), 3)
CI_AUC_std <- pROC::ci.auc(ROC_AUC_std_DAY7, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)
AUC_anotherpart_std <- sub(".*(: *)", "//1", CI_AUC_std)
AUC_secondpart_std <- as.numeric(substr(AUC_anotherpart_std[1], 1, 5))
AUC_thirdpart_std <- as.numeric(substr(AUC_anotherpart_std[3], 1, 5))
AUC_label_AUC_BW1_DAY7 <- paste0("AUC:", AUC_firstpart_std, "(", AUC_secondpart_std, "-", AUC_thirdpart_std, ")", sep = "")

# For recommended dosing - DAY 7
analysis_DAY7 <- pop_opt_dos %>% filter(DAY == 2) %>% mutate(TA = ifelse(fAUC >= 200, 1, 0))

ROC_AUC_opt_DAY7 <- pROC::roc(analysis_DAY7$TA,
                        analysis_DAY7$BW,
                        auc.polygon = FALSE, max.auc.polygon = FALSE, grid = FALSE,
                        ci = TRUE, boot.n = 1000, ci.alpha = 0.95, stratified = TRUE,
                        plot = FALSE, print.auc = TRUE, print.auc.x = 0.2, print.auc.y = 0.3,
...
AUC_opt <- pROC::auc(ROC_AUC_opt_DAY7, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)
AUC_firstpart_opt <- round(as.numeric(sub(".*(: *)", "//1", AUC_opt)), 3)
CI_AUC_opt <- pROC::ci.auc(ROC_AUC_opt_DAY7, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)
AUC_anotherpart_opt <- sub(".*(: *)", "//1", CI_AUC_opt)
AUC_secondpart_opt <- as.numeric(substr(AUC_anotherpart_opt[1], 1, 5))
AUC_thirdpart_opt <- as.numeric(substr(AUC_anotherpart_opt[3], 1, 5))
AUC_label_AUC_BW_DAY7 <- paste0("AUC:", AUC_firstpart_opt, "(", AUC_secondpart_opt, "-", AUC_thirdpart_opt, ")", sep = "")

# Convert roc objects to data frames
roc1_DAY7 <- pROC::coords(ROC_AUC_std_DAY7)
roc_DAY7 <- pROC::coords(ROC_AUC_opt_DAY7)

# Plot ROC curves - DAY 7
Plot_ROC_AUC_opt_DAY7 <- ggplot() +
...
# Combine the annotation and plot using grid.arrange
combined_plot_DAY7 <- grid.arrange(annotation, Plot_ROC_AUC_opt_DAY7, ncol = 1, heights = c(0.02, 0.98))
...
analysis1_DAY14 <- sdtab_pop_std_2706_trough %>% filter(DAY == 14) %>% mutate(TA = ifelse(fAUC >= 200, 1, 0))

ROC_AUC_std_DAY14 <- pROC::roc(analysis1_DAY14$TA,
                        analysis1_DAY14$BW,
                        auc.polygon = FALSE, max.auc.polygon = FALSE, grid = FALSE,
                        ci = TRUE, boot.n = 1000, ci.alpha = 0.95, stratified = TRUE,
                        plot = FALSE, print.auc = TRUE, print.auc.x = 0.2, print.auc.y = 0.3,
...
AUC_std <- pROC::auc(ROC_AUC_std_DAY14, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)
AUC_firstpart_std <- round(as.numeric(sub(".*(: *)", "//1", AUC_std)), 3)
CI_AUC_std <- pROC::ci.auc(ROC_AUC_std_DAY14, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)
AUC_anotherpart_std <- sub(".*(: *)", "//1", CI_AUC_std)
AUC_secondpart_std <- as.numeric(substr(AUC_anotherpart_std[1], 1, 5))
AUC_thirdpart_std <- as.numeric(substr(AUC_anotherpart_std[3], 1, 5))
AUC_label_std_DAY14 <- paste0("AUC:", AUC_firstpart_std, "(", AUC_secondpart_std, "-", AUC_thirdpart_std, ")", sep = "")

# For recommended dosing - DAY 14
analysis_DAY14 <- pop_opt_dos %>% filter(DAY == 2) %>% mutate(TA = ifelse(fAUC >= 200, 1, 0))

ROC_AUC_opt_DAY14 <- pROC::roc(analysis_DAY14$TA,
                        analysis_DAY14$BW,
                        auc.polygon = FALSE, max.auc.polygon = FALSE, grid = FALSE,
                        ci = TRUE, boot.n = 1000, ci.alpha = 0.95, stratified = TRUE,
                        plot = FALSE, print.auc = TRUE, print.auc.x = 0.2, print.auc.y = 0.3,
...
AUC_opt <- pROC::auc(ROC_AUC_opt_DAY14, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)
AUC_firstpart_opt <- round(as.numeric(sub(".*(: *)", "//1", AUC_opt)), 3)
CI_AUC_opt <- pROC::ci.auc(ROC_AUC_opt_DAY14, conf.level = 0.95, ci.alpha = 0.95, method = "bootstrap", boot.n = 1000, stratified = TRUE)
AUC_anotherpart_opt <- sub(".*(: *)", "//1", CI_AUC_opt)
AUC_secondpart_opt <- as.numeric(substr(AUC_anotherpart_opt[1], 1, 5))
AUC_thirdpart_opt <- as.numeric(substr(AUC_anotherpart_opt[3], 1, 5))
AUC_label_opt_DAY14 <- paste0("AUC:", AUC_firstpart_opt, "(", AUC_secondpart_opt, "-", AUC_thirdpart_opt, ")", sep = "")

# Convert roc objects to data frames
roc_std_DAY14 <- pROC::coords(ROC_AUC_std_DAY14)
roc_opt_DAY14 <- pROC::coords(ROC_AUC_opt_DAY14)

# Plot ROC curves - DAY 14
Plot_ROC_AUC_opt_DAY14 <- ggplot() +
  ...
  geom_segment(aes(x = 0, xend = 1, y = 0, yend = 1), color = "black", linetype = "solid") +
  annotate(geom = "text", x = 0.60, parse = TRUE, y = 0.30, size = 4, label = AUC_label_std_DAY14, color = "orange") +
  annotate(geom = "text", x = 0.60, parse = TRUE, y = 0.10, size = 4, label = AUC_label_opt_DAY14, color = "steelblue") +
  guides(color = guide_legend(override.aes = list(linetype = c("solid", "dashed"))), linetype = "none") +
        ggtitle("DAY 14") + 
...
# Combine the annotation and plot using grid.arrange
combined_plot_DAY14 <- grid.arrange(annotation, Plot_ROC_AUC_opt_DAY14, ncol = 1, heights = c(0.02, 0.98))
...
# Save the combined plots
#setwd("D:/Projects/Fluconazole PoPPK KU Leuven/Fluconazol_project/Bootstrap and dosing simulation/BW_Simulations/Manuscript_plots")
#ggsave("roc_opt_DAY14.png", combined_plot_DAY14, dpi = 300, width = 8.27, height = 10.3, units = "in", limitsize = FALSE)
...
setwd("D:/Projects/Fluconazole PoPPK KU Leuven/Fluconazol_project/Bootstrap and dosing simulation/BW_Simulations/Manuscript_plots")
ggsave("combined_roc_plot_2706.png", combined_plot_roc, dpi = 300, width = 8.27, height = 10.3, units = "in", limitsize = FALSE)

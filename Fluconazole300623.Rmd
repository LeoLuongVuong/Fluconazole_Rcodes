...
# I hid this as it is not relevant anymore 280823
...
# Creating a CKDEPI_DOS_INT column 060923

FlucTotIV_clean$CKDEPI_DOS_INT <- ifelse(FlucTotIV_clean$SEX == 1 & FlucTotIV_clean$CREAT_DOS_INT < 0.9,(142) * ((FlucTotIV_clean$CREAT_DOS_INT/0.9)^-0.302) * (0.9938^FlucTotIV_clean$AGE), ifelse(FlucTotIV_clean$SEX == 1 & FlucTotIV_clean$CREAT_DOS_INT >= 0.9,(142) * ((FlucTotIV_clean$CREAT_DOS_INT/0.9)^-1.200) * (0.9938^FlucTotIV_clean$AGE), ifelse(FlucTotIV_clean$SEX == 2 & FlucTotIV_clean$CREAT_DOS_INT < 0.7, (142) * ((FlucTotIV_clean$CREAT_DOS_INT/0.7)^-0.241) * (0.9938^FlucTotIV_clean$AGE)*1.012, (142) * ((FlucTotIV_clean$CREAT_DOS_INT/0.7)^-1.200) * (0.9938^FlucTotIV_clean$AGE)*1.012)))

# Creating a CKDEPI column 060923
...
# between dosing interval variability in CKDEPI_DOS_INT of a patient
bet_int_var <- FlucTotIV_clean %>%
  filter(unique_CREAT == 1) %>%
  group_by(ID) %>%
  summarize(delta_ckdepi = max(CKDEPI_DOS_INT, na.rm = TRUE) - min(CKDEPI_DOS_INT, na.rm = TRUE))

# between dosing interval variability in CKDEPI_DOS_INT of a patient, excluding unique_CREAT == 0 (or we can consider within a patient as a whole)
...
# between interval variability (biv) aggregating on ID
biv<-mean(bet_int_var$delta_ckdepi[bet_int_var$delta_ckdepi!=0&bet_int_var$delta_ckdepi!=-Inf])
biv #22.2518
...
# Adding a term to exclude NaN, but it seems not necessary! - 060923
average_excluding_zero <- bet_int_var_imputed %>%
  summarize(across(starts_with("CKDEPI"), ~ mean(.[. != 0 & !is.nan(.)], na.rm = TRUE)))

# View the resulting average
View(average_excluding_zero)

# exporting average_excluding_zero of 2l.pan method 050923
setwd("D:/Projects/Fluconazole PoPPK KU Leuven/Fluconazol_project/My datasets and R codes/Imputed_dos_int_datasets")
#write.csv(average_excluding_zero, "BIV_2lpan_ID.csv",quote=F,row.names = FALSE) #BIV = Between Interval Variability
#write.csv(average_excluding_zero, "BIV_2lpmm_ID.csv",quote=F,row.names = FALSE)
write.csv(average_excluding_zero, "BIV_2lpmm_ID_metric.csv",quote=F,row.names = FALSE)
write.csv(average_excluding_zero, "BIV_2lnorm.csv",quote=F,row.names = FALSE)
...
### Imputing with mice.impute.2l.pan function 280823

#### Exporting the dataset ready to be analysed in NONMEM 280823
#### Will save it under the same name FlucTotIV_clean and the previous version is moved to the archive folder

```{r exporting FlucTotIV_clean dataset}
setwd("D:/Projects/Fluconazole PopPK KU Leuven/Fluconazol_project/My datasets and R codes")
...
### Doing this on 01/09/23
setwd("D:/Projects/Fluconazole PopPK KU Leuven/Fluconazol_project/My datasets and R codes")
FlucTotIV_clean <- read.csv("FlucTotIV_clean.csv", na.strings = "NA")

```

#### Performing the linear-mixed effect MI
...
##### Another imputation strategy - 1 dataset for all - 060923

```{r best imputation overall dataset}

### I add ID to the model in accordance with the thesis I cited below 280823
impute_CREAT_DOS_INT <- FlucTotIV_clean[, c("ID","TIME","RATE","AMT","DV","AGE","SEX","BW2","LENGTH", "CRRT","OCC","HOSPITAL","CREAT_DOS_INT")]

```

##### Then imputing crrt and non-crrt models using mice.impute.2l.pan function

##### These codes are based on page 112 Imputation of Missing Data in Chronic Hepatitis C Patient Utility Data thesis - University of Guelph

##### Also based on chapter 7.6 Van Buuren's book
...
predmat <- imp0$predictorMatrix
predmat["CREAT_DOS_INT","HOSPITAL"] <- -2
meth <- imp0$method
...
```{r for impute_CREAT_DOS_INT_crrt dataset 2l.norm method}
...
```{r for imputed_CREAT_DOS_INT_nocrrt dataset 2l.norm method}
...
##### Clustering inside ID, instead of HOSPITAL as above
...
##### Imputation overall dataset 060923

```{r for impute_CREAT_DOS_INT dataset 2l.pmm method,warning=FALSE}

# Adapted 050923

library(miceadds)
set.seed(123)
imp0 <- mice(impute_CREAT_DOS_INT, maxit=0)
predmat <- imp0$predictorMatrix
predmat["CREAT_DOS_INT","ID"] <- -2
meth <- imp0$method
meth["CREAT_DOS_INT"] <- "2l.pmm"
maxit <- 20
nimp <- 10
imputed_CREAT_DOS_INT <- mice::mice(data=impute_CREAT_DOS_INT,method=meth,predictorMatrix = predmat,maxit=maxit, m=nimp, printFlag = FALSE)

FlucTotIV_clean_imputed <- FlucTotIV_clean

for (i in 1:10) {
        FlucTotIV_clean_imputed[[paste0("CREAT", sprintf("%02d", i))]] <- (complete(imputed_CREAT_DOS_INT, action = i))$CREAT_DOS_INT
}

```

##### Helpful mice tutorial from youtube - 050923

```{r youtube mice tutorial}
...
# Applying to my dataset
md.pattern(impute_CREAT_DOS_INT_crrt)
# Interpreting the plot
# - 49 obs where only CREAT_DOS_INT is missing
# - 125 obs where only DV is missing
# - 160 obs with both DV and CREAT_DOS_INT missing
# - 89 obs with BW2, LENGTH and CREAT_DOS_INT missing
# - 85 obs with BW2, LENGTH, DV and CREAT_DOS_INT missing
...
densityplot(imputed_CREAT_DOS_INT_crrt) #good plot
densityplot(imputed_CREAT_DOS_INT_nocrrt)
stripplot(imputed_CREAT_DOS_INT_crrt) #good plot

# checking if convergence is achieved
plot(imputed_CREAT_DOS_INT_crrt) # no trend is observed, good!

# Which methods were used?
imp.data
imputed_CREAT_DOS_INT_crrt # all pmm method
...
imputed_CREAT_DOS_INT_crrt2<- mice(impute_CREAT_DOS_INT_crrt, m = 10,maxit=20,method=my.method,seed=123,printFlag = FALSE)
...
##### Two level imputation chapter 7.6 - Van Buuren's book - 050923

```{r trying 2l.pmm and 2l.pan (2l.norm) method as in the example}
library(mice)
library(miceadds) #to use 2l.pmm model

methods <- c("2l.pan", "2l.pmm")
result <- vector("list", length(methods))
names(result) <- methods
for (meth in methods) {
 impute_CREAT_DOS_INT_crrt <- FlucTotIV_clean_crrt[, c("TIME","RATE","AMT","DV","AGE","SEX","BW2","LENGTH", "CRRT","OCC","HOSPITAL","CREAT_DOS_INT")]
  pred <- make.predictorMatrix(impute_CREAT_DOS_INT_crrt)
  pred["HOSPITAL"] <- -2
  result[[meth]] <- mice(impute_CREAT_DOS_INT_crrt, pred = pred, meth = meth,
                         m = 10, maxit = 20, print = FALSE, seed = 123)
}

```

##### Diagnostic plots for MI 130923
...
ggplot(FlucTotIV_clean_imputed, aes(x = CREAT01)) + 
        ...
        geom_density(aes(x = CREAT09, fill = "CREAT09"), alpha = 0.5) + 
        geom_density(aes(x = CREAT10, fill = "CREAT10"), alpha = 0.5) +
        geom_density(aes(x = CREAT, fill = "CREAT"), alpha = 0.5) +
        scale_fill_manual(name = "Variable", values = c("CREAT01" = "blue", "CREAT02" = "lightblue", "CREAT03" = "green",
                                                        "CREAT04" = "orange", "CREAT05" = "purple", "CREAT06" = "pink",
                                                        "CREAT07" = "brown", "CREAT08" = "gray", "CREAT09" = "black",
                                                        "CREAT10" = "yellow","CREAT" = "red")) +
        xlab("Creatinine Levels") +
        ylab("Density") +
        ggtitle("Density Plot of 10 imputed creatinin vs the original creatinin")
...
##### Consider DV as a level 1 predictor for CREAT and adds its mean value to the imputation model 130923

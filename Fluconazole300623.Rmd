...
# Sort the dataset by ID and OCC
FlucTotIV_clean <- FlucTotIV_clean %>%
  arrange(ID, OCC)

# Group by ID and OCC and fill missing CREAT2 with observed CREAT2
FlucTotIV_clean <- FlucTotIV_clean %>%
  group_by(ID, OCC) %>%
  mutate(dosing_interval = ifelse(is.na(dosing_interval), na.omit(dosing_interval), dosing_interval)) %>%
  ungroup()
...
sum(!is.na(FlucTotIV_clean$CREAT3)) #864
...
###### Building imputation models based on CRRT status (whether a patient has at least 1 CRRT session), and the predictors that I will be using will be restricted to "TIME", "AMT", "RATE", "DV", "AGE", "BW2", "SEX", "LENGTH", "HOSPITAL", "OCC", "TAD", "CRRT", "CREAT2", "dosing_interval" 

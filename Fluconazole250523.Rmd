...
PTA_CRRT200_DAY1<- PTA_overall %>%
  ...
  theme(legend.position = "right") +
  theme(plot.title = element_text(hjust = 0.5, size = 10,face="bold"),
        axis.title = element_text(size = 8),legend.title = element_text(size = 7),
        axis.text = element_text(size = 7),
        legend.text = element_text(size = 6),
        panel.grid.major = element_line(colour = "gray85"),
        panel.grid.minor = element_blank()) +
...
ggsave("CRRT_pooled.png", CRRT_pooled, dpi = 300, width = 26, height = 13.44,units = "cm", limitsize = FALSE)
...
ggsave("noCRRT_pooled.png", noCRRT_pooled, dpi = 300, width = 26, height = 13.44,units = "cm", limitsize = FALSE)

```

#### Creating datasets for population simulation 010623
...
summary(as.factor(pop_dosing$TAD))
...
selected_rows <- FlucTotIV_clean %>%
  filter(TAD >= 23, TAD <= 25, DV > 50) %>%
  distinct(ID)
...
# Filter rows for the selected patients
selected_patients <- c(117, 122, 126, 139, 144)
selected_data <- FlucTotIV_clean %>%
  filter(ID %in% selected_patients)

# Remove NAs from DV column
selected_data <- selected_data %>%
  drop_na(DV)
selected_data$CRRT<-as.factor(selected_data$CRRT)

# Create line plots with geom_points
plots <- lapply(selected_patients, function(patient_id) {
  data <- selected_data %>%
